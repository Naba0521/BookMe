...
import {
  ...
  OrderType,
  TimingProps,
} from "../../(publicItems)/_OrderPageTypes/types";
...
function StageTwoTimePicking({
  ...
  const getEmployee = company?.employees.find(
  ...
  const Name = getEmployee?.employeeName;
  console.log(Name);
  ...
  const allSelectedTimes = orders
    ...
    : [];
  const isFully = availabilityTimes().every((time) =>
